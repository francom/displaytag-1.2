...
public class TableTag extends HtmlTableTag
{
    ...
    public int doStartTag() throws JspException
    {
        ...
        //setting default pagesize value, from displaytag.properties, key "pagination.pagesize" in case the corresponding attribute is not specified
        //if it is present and is set to -1, then no pagination is implemented
        if (this.pagesize == 0) 
        {
            this.pagesize = properties.getPaginationPageSize();
        } 
    ...
    private void initParameters() throws JspTagException, FactoryInstantiationException
    {
        ...
        //in case of records deletion and keep status set to true, the value of the pageNumber 
		//field must be checked and consequently updated 
        if (this.pagesize > 0
                && keepStatus
                && (this.pageNumber > ((Collection) this.list).size()
                        / this.pagesize)
                && (((Collection) this.list).size() % this.pagesize == 0))
        {
            this.pageNumber = ((Collection) this.list).size() / this.pagesize;
        }
        ...
        if (this.partialList 
                //added to manage fullList in case of export
